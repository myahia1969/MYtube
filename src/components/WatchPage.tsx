...
import { ThumbsUp, ThumbsDown, Share2, CornerDownRight, MoreHorizontal, Send, Play, Clock, AlertCircle, ExternalLink, Download, X, Sparkles, Loader2, Brain, MessageSquare, User as UserIcon, Bookmark, Trash2, Wand2, Compass, Filter, Mail, Copy, Check } from 'lucide-react';
...
interface WatchPageProps {
  ...
  watchLater?: string[];
  onToggleWatchLaterId?: (videoId: string) => void;
...
export default function WatchPage({
  ...
  history = [],
  watchLater = [],
  onToggleWatchLaterId,
  onShare,
}: WatchPageProps) {
  const [commentInput, setCommentInput] = useState('');
  const [showShareMenu, setShowShareMenu] = useState(false);
  const [copiedLink, setCopiedLink] = useState(false);
  const [recFilter, setRecFilter] = useState<'all' | 'ai' | 'channel' | 'category'>('all');
  ...
  // Filter out the current active video and apply active sidebar recommendation filter
  const baseRecommended = allVideos.filter((v) => v.id !== video.id);
  const recommendedVideos = baseRecommended.filter((v) => {
    if (recFilter === 'channel') return v.channelId === video.channelId;
    if (recFilter === 'category') return v.category === video.category;
    if (recFilter === 'ai') return v.category === video.category || v.views > 5000;
    return true;
  });
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 text-[#0f0f0f] p-4 max-w-7xl mx-auto w-full">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 text-xs font-sans text-gray-500">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="relative">
                ...
                {showShareMenu && (
                  <div className="absolute right-0 top-full mt-2 w-56 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-xl p-1.5 z-30 animate-in fade-in zoom-in-95">
                    <button
                      onClick={() => {
                      ...
                      className="w-full flex items-center gap-2.5 px-3 py-2 text-xs font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-xl transition-colors text-left cursor-pointer"
                    >
                      {copiedLink ? (
                      ...
                      <span>
                        {copiedLink
                          ...
                          : (language === 'ar' ? 'نسخ رابط الفيديو' : 'Copy Video Link')}
                      </span>
                    </button>
                    ...
                    >
                      ...
                      <span>{language === 'ar' ? 'المشاركة عبر البريد (Email)' : 'Share via Email'}</span>
                    </button>
                  </div>
                )}
              </div>
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <h3 className="font-sans font-bold text-sm text-gray-900 flex items-center gap-1.5">
              <span>{language === 'ar' ? 'فيديوهات مقترحة' : 'Recommended Videos'}</span>
              <span className="text-[10px] bg-gray-100 text-gray-600 font-mono px-2 py-0.5 rounded-full font-bold">
                {recommendedVideos.length}
              </span>
            </h3>
            <span className="text-[10px] text-gray-400 font-mono">{language === 'ar' ? 'التالي' : 'Next Up'}</span>
          </div>
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto custom-scrollbar pb-1 text-[11px] font-bold">
            ...
            <button
              onClick={() => setRecFilter('ai')}
              className={`px-2.5 py-1 rounded-lg transition-all whitespace-nowrap cursor-pointer flex items-center gap-1 ${
                recFilter === 'ai'
            ...
            >
              ...
              <span>{language === 'ar' ? 'توصيات ذكية' : 'AI Picks'}</span>
            </button>
            <button
            ...
            >
              {language === 'ar' ? 'نفس الفئة' : 'Same Category'}
            </button>
            <button
              onClick={() => setRecFilter('channel')}
              className={`px-2.5 py-1 rounded-lg transition-all whitespace-nowrap cursor-pointer ${
                recFilter === 'channel'
                  ? 'bg-gray-900 text-white shadow-sm'
                  : 'bg-gray-100 hover:bg-gray-200 text-gray-600'
            ...
            >
              {language === 'ar' ? 'نفس القناة' : 'Same Channel'}
            </button>
          </div>
        </div>
        ...
        <div className="space-y-3 max-h-[85vh] overflow-y-auto pr-1">
          {recommendedVideos.length === 0 ? (
            <div className="p-6 text-center bg-gray-50 rounded-2xl border border-dashed border-gray-200 text-gray-500 text-xs">
              {language === 'ar' ? 'لا توجد فيديوهات مطابقة لهذا الفلتر' : 'No videos match this recommendation filter'}
            </div>
          ) : (
            recommendedVideos.map((recVideo) => (
              <div
                ...
                className="flex gap-3 p-2 bg-white hover:bg-gray-50 border border-gray-200/90 hover:border-red-200 rounded-xl cursor-pointer group transition-all relative overflow-hidden"
              >
                {/* Thumbnail half */}
                ...
                <div className="min-w-0 flex flex-col justify-center gap-0.5 flex-1">
                  <div className="flex items-center gap-1">
                    {recVideo.category === video.category && (
                      <span className="text-[9px] bg-red-50 text-red-600 px-1.5 py-0.2 rounded font-mono font-bold">
                        {recVideo.category}
                      </span>
                    )}
                  </div>
                  <h4 className="font-sans font-semibold text-xs text-gray-900 line-clamp-2 leading-tight group-hover:text-red-600 transition-colors">
                    {recVideo.title}
                  </h4>
                  ...
                  <div className="flex items-center gap-1.5 text-[9px] text-gray-400 font-mono mt-0.5">
                    ...
                    <span className="text-emerald-600 font-bold">
                      {recVideo.category === video.category ? '96% match' : '88% match'}
                    </span>
                  </div>
                </div>
              </div>
